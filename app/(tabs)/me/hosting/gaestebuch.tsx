...
export default function GaestebuchScreen() {
  ...
  const fetchAll = useCallback(async () => {
    ...
    try {
      ...
      const list = (entryData as GaestebuchRow[]) || [];
      setEntries(list);
      ...
      console.error('fetch hosting gaestebuch failed', err);
    } finally {
      setLoading(false);
      setRefreshing(false);
    }
  ...
  const handlePostReply = async (entryId: string) => {
    ...
    if (!canPerformAction('post-reply-host', 1500)) return;
  ...
  const avgRating = entries.length
    ? (entries.reduce((sum, e) => sum + (e.rating ?? 0), 0) / Math.max(1, entries.filter(e => e.rating).length)) || 0
    : 0;

  const renderEntry = ({ item }: { item: GaestebuchRow }) => {
        ...
        {open ? (
          <View style={styles.replyComposer}>
            <TextInput
              ...
              placeholder="Thank them, share a memory…"
            ...
            <View style={styles.replyActions}>
              <TouchableOpacity onPress={() => { setOpenReply(null); setReplyText(''); }} style={styles.replyCancel}>
                <Text style={styles.replyCancelText}>Cancel</Text>
              </TouchableOpacity>
              <TouchableOpacity
                onPress={() => handlePostReply(item.id)}
                disabled={!replyText.trim() || sending || replyText.length > REPLY_MAX}
                style={[styles.replySend, (!replyText.trim() || sending || replyText.length > REPLY_MAX) && { opacity: 0.4 }]}
              >
          ...
          >
            ...
            <Text style={styles.replyBtnText}>{entryReplies.length > 0 ? 'Add another reply' : 'Reply'}</Text>
          </TouchableOpacity>
        )}
      </WKCard>
    );
    ...
    <SafeAreaView style={styles.container} edges={['top']}>
      ...
      >
        ...
        <FlatList
          ...
          refreshControl={<RefreshControl refreshing={refreshing} onRefresh={() => { setRefreshing(true); fetchAll(); }} tintColor={colors.amber} />}
          ListEmptyComponent={
...
const styles = StyleSheet.create({
  ...
  replyAvatar: { width: 24, height: 24, borderRadius: 12, backgroundColor: colors.surfaceAlt, alignItems: 'center', justifyContent: 'center', overflow: 'hidden' },
  replyAvatarImg: { width: '100%', height: '100%' },
  replyBubble: { flex: 1, backgroundColor: colors.surfaceAlt, borderRadius: 8, paddingHorizontal: 10, paddingVertical: 6 },
  ...
  replyBtn: { flexDirection: 'row', alignItems: 'center', gap: 6, marginTop: spacing.md, paddingVertical: 6 },
  replyBtnText: { fontSize: 12, fontWeight: '600', color: colors.amber, letterSpacing: 0.4 },

  replyComposer: { marginTop: spacing.md, padding: 10, backgroundColor: colors.amberBg, borderRadius: 8, borderWidth: 1, borderColor: colors.amberLine },
  replyInput: { fontSize: 13, color: colors.ink, minHeight: 38, maxHeight: 100, backgroundColor: '#fff', borderRadius: 6, paddingHorizontal: 10, paddingVertical: 8, marginBottom: 8 },
  ...
  replySend: { flexDirection: 'row', alignItems: 'center', gap: 4, backgroundColor: colors.amber, paddingHorizontal: 12, paddingVertical: 6, borderRadius: 6 },
